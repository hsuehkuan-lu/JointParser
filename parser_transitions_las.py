...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        """
        tran, label = transition
        # shift
        if tran == 'S':
            self.stack.append(self.buffer.pop(0))
        elif tran == 'LA':
            head, dependent = self.stack[-1], self.stack[-2]
            self.dependencies.append((head, dependent, label))
            self.stack.pop(-2)
        elif tran == 'RA':
...
import random
def minibatch_parse(sentences, model, batch_size, config):
    ...
    while unfinished_parses:
        ...
        transitions = model.predict(mini_batch, config)
